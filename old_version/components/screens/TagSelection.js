...
class Tag extends Component {
  ...
  async readUserData(UserId) {
    const tmp = await fire.database().ref(`UsersTest/${UserId}`).once('value', (snapshot) => snapshot.val());
    return tmp;
  }
  ...
  async writeUserData(UserId, tag) {
    ...
    const val = await this.readUserData(UserId);
...
export default class TageSelection extends React.Component {
  ...
  async readTagsData() {
    const tmp = await fire.database().ref('Tags/').once('value', (snapshot) => snapshot.val());
    console.log('tmp: ', tmp);
    return tmp;
  }

  async displayTags() {
    const tmp = await this.readTagsData();
    this.updateState(tmp);
    const test = [];
    let i = 0;
    this.state.tags.forEach((childSnapshot) => {
      const item = childSnapshot.val();
      test.push({
        key: i, name: item.name, rank: item.rank, pressed: false
      });
      i++;
    });
  ...
  async NextPage(UserId) {
    const tmp = await this.readUserData(UserId);
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <View style={{ flex: 1 }}>
          <ImageBackground source={require('../../assets/london2.jpg')} style={{ width: Dimensions.get('window').width, height: Dimensions.get('window').width / 2.2 - 10, }} />
        </View>
        ...
        >
          ...
          <Text style={[{ textAlign: 'center', fontWeight: 'bold', fontSize: 35 }]}>{navigation.getParam('name')}</Text>
          <Text style={[{
            textAlign: 'center', color: 'grey', fontSize: 20, marginTop: 20, fontWeight: 'light'
          }]}
          >
        ...
        <View style={{ flex: 2, margin: 10, marginTop: 20 }}>
          ...
          <Button
            color="#89B3D9"
            title="Next"
            onPress={() => this.NextPage(navigation.getParam('uid'))}
          />
        </View>
      </View>
    );
